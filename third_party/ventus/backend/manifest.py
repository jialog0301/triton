...
def _invocation(executable: Path, argv, result: subprocess.CompletedProcess,
                identity: str):
...
def toolchain(identity: dict):
    ...
    for field, component in (("llvm", "llvm_commit"), ("driver", "driver_commit"),
                             ("spike", "spike_commit"),
                             ("cycle_sim", "cyclesim_commit"),
                             ("rtl_simulator", "gpgpu_commit")):
    ...
    def stage(self, ext: str) -> Path:
        ...
        if len(matches) != 1:
            raise FileNotFoundError(
                f"expected one .{ext} artifact in {self.cache_dir}, got {matches}")
        return matches[0]
...
def _elf_identity(elf_path: Path, kernel_name: str):
    ...
    header = subprocess.run([str(readelf), "-h", str(elf_path)],
    ...
    record.validated = (record.elf_class == 32 and record.endianness == "little"
                        and record.machine == "EM_RISCV")
    return record
...
def _raw_resource(elf_path: Path, kernel_name: str):
    ...
    readelf = Path(load_identity()["tool_binary_hashes"]["llvm-readobj"]["path"]
                   ).with_name("llvm-readelf")
    sections = subprocess.run([str(readelf), "-S", str(elf_path)],
                              capture_output=True, text=True, check=True).stdout
    import re
    ...
    if not match:
        raise FileNotFoundError(
            f"{elf_path}: no .ventus.resource.{kernel_name} section")
...
def build_compile_manifest(facts: CompileFacts, manifest_path: Path,
                           identity=None, tmp_dir: Path | None = None):
    ...
    tools = {name: Path(entry["path"])
             for name, entry in identity["tool_binary_hashes"].items()}
    tmp_dir = Path(tmp_dir) if tmp_dir else Path(
        tempfile.mkdtemp(prefix="ventus-manifest-"))
    ...
    opt_result = subprocess.run([str(a) for a in opt_argv],
                                capture_output=True, text=True)

    # Gate 3: target codegen to an object.
    obj = tmp_dir / "kernel.o"
    llc_argv = [tools["llc"], f"-mtriple={identity['target_triple']}",
                f"-mcpu={identity['mcpu']}", "-filetype=obj", verified,
                "-o", obj]
    llc_result = subprocess.run([str(a) for a in llc_argv],
                                capture_output=True, text=True)

    asm = tmp_dir / "kernel.s"
    asm_result = subprocess.run(
        [str(tools["llc"]), f"-mtriple={identity['target_triple']}",
         f"-mcpu={identity['mcpu']}", "-filetype=asm", str(verified),
         "-o", str(asm)], capture_output=True, text=True)
    ...
    md.resources.range_validated = (vgpr <= VGPR_LIMIT and sgpr <= SGPR_LIMIT
                                    and lds_bytes <= LDS_LIMIT_BYTES
    ...
    md.capability.simulator_address_convention = identity[
        "simulator_address_convention"]
    md.capability.rtl_profile_hash = md.rtl_profile.content_hash
    md.capability.completion_contract_version = identity[
    ...
    md.compatibility.opt = _invocation(tools["opt"], opt_argv, opt_result,
                                       tool_identity(identity, "opt"))
    md.compatibility.llc = _invocation(tools["llc"], llc_argv, llc_result,
                                       tool_identity(identity, "llc"))
    ...
    link_inputs = []
    for kind, key in (("linker_script", "ventus_linker_script"),
                      ("crt0", "ventus_crt0_input"),
                      ("libclc", "ventus_libclc_input"),
                      ("workitem", "ventus_workitem_input")):
        ref = ventus.ArtifactReference()
        ref.kind = kind
    ...
    md.link.lld = _invocation(tools["ld.lld"],
                              [tools["ld.lld"], "-T",
                               identity["ventus_linker_script"]["path"],
                               "-o", elf],
                              subprocess.CompletedProcess([], 0, "", ""),
                              tool_identity(identity, "ld.lld"))
    md.link.elf_hash = md.elf.content_hash
    md.link.validated = md.elf.validated and md.compatibility.internal_check_passed

    compile_side = [("ttir", ttir), ("ttgir", ttgir), ("llvm_ir", verified),
                    ("assembly", asm), ("elf", elf)]
    md.artifacts = [_artifact_ref(kind, path) for kind, path in compile_side
                    if path.is_file()]
...
def record_execution(md, manifest_path: Path, launcher_inputs, result: dict,
    ...
    """
    elf_ref = next(a for a in md.artifacts if a.kind == "elf")
    elf_path = Path(elf_ref.path)

    launcher_input = manifest_path.with_suffix(".launcher.json")
    launcher_input.write_text(json.dumps(launcher_inputs, indent=2,
                                         sort_keys=True))
    launcher_ref = _artifact_ref("launcher_input", launcher_input)

    test_result = manifest_path.with_suffix(".result.json")
    test_result.write_text(json.dumps(result, indent=2, sort_keys=True,
                                      default=str))
    result_ref = _artifact_ref("test_result", test_result)

    kept = [a for a in md.artifacts
            if a.kind not in ("launcher_input", "test_result")]
    kept.extend((launcher_ref, result_ref))
    md.artifacts = kept

    md.result_records = [
        r for r in md.result_records if not r.startswith("gate4=")
    ] + [
...
def validate(md, identity=None) -> bool:
    return ventus.VentusKernelMetadata.validate(
        md, validation_context(identity or load_identity()))
